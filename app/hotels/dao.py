...
class HotelDAO(BaseDAO):
    ...
    async def find_all(
        ...
        booked_hotels = select(Rooms.hotel_id, func.sum(Rooms.quantity-func.coalesce(booked_rooms.c.occupied_rooms, 0)).label("rooms_left")).select_from(Rooms).join(booked_rooms, booked_rooms.c.room_id == Rooms.id, isouter=True).group_by(Rooms.hotel_id).cte("booked_hotels")

        get_hotels_with_rooms_left = select(Hotels.__table__.columns, booked_hotels.c.rooms_left).select_from(Hotels).join(booked_hotels, Hotels.id == booked_hotels.c.hotel_id, isouter=True).where(and_(Hotels.location.like(f"%{location}%"), booked_hotels.c.rooms_left >=1))
        ...
        async with async_session_maker() as sessesion:
            ...
            hotels_with_rooms_left = await sessesion.execute(get_hotels_with_rooms_left)
